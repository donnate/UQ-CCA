...
get_rrr_simulation_api <- function(ccar3_code_path = NULL) {
  ccar3_code_path <- ccar3_code_path %||% default_ccar3_code_path()
  if (is.null(ccar3_code_path) || !nzchar(ccar3_code_path)) {
    stop(
      paste(
        "Simulation code path is not configured.",
        "Set `options(uqcca.ccar3_code_path = \"/path/to/CCAR3_code\")`",
        "or define the `UQCCA_CCAR3_CODE_PATH` environment variable."
      ),
      call. = FALSE
    )
  }
...
#' Generate synthetic reduced-rank regression data for UQ benchmarking
#'
#' Wraps the external simulation generator used by the original project and
#' returns both the observed data matrices and the ground-truth canonical
#' directions/regression coefficients needed for coverage and FDR evaluation.
#'
#' @param n Number of paired samples to generate.
#' @param p,q Numbers of features in the X and Y blocks.
#' @param r Rank of the latent canonical signal.
#' @param nnzeros Number of nonzero loadings per canonical direction in the
#'   underlying generator.
#' @param theta_strength Either a named signal-strength setting (`"high"`,
#'   `"medium"`, `"low"`), a scalar, a length-`r` vector, or an `r`-by-`r`
#'   matrix controlling the population canonical correlations.
#' @param r_pca,lambda_pca,overlapping_amount,normalize_diagonal,n_new
#'   Additional arguments forwarded to the external simulation generator.
#' @param ccar3_code_path Optional path to the external `CCAR3_code`
#'   repository containing the simulation scripts.
#' @param generator_api Optional preloaded simulation environment returned by
#'   the internal generator loader.
#'
#' @return A list containing simulated `X` and `Y` matrices, optional holdout
#'   matrices, the ground-truth canonical directions and regression targets, and
#'   the settings used to generate the dataset.
#' @export
...
#' Run the reduced-rank regression simulation benchmark
#'
#' Repeats the synthetic-data generator, sparse CCA fit, bootstrap UQ, and
#' cross-fitted inference pipeline across multiple replications, then summarizes
#' coverage, power, false discovery, and subspace recovery.
#'
#' @inheritParams generate_rrr_simulation_data
#' @inheritParams fit_ecca_cv
#' @inheritParams crossfit_cca_inference
#' @param n_sims Number of independent simulation replications.
#' @param lambdas Candidate penalties passed to [fit_ecca_cv()].
#' @param n_boot Number of bootstrap replicates used within each simulation.
#' @param alpha Two-sided error level used for interval evaluation.
#' @param truth_tol Absolute threshold used to decide whether a ground-truth
#'   coefficient counts as nonzero.
#' @param bootstrap_threshold Absolute threshold used when computing bootstrap
#'   selection stability.
#' @param strength_bins Number of bins used when aggregating power by signal
#'   strength.
#' @param seed Base random seed for the benchmark.
#' @param crossfit_variance_shrinkage Residual-variance shrinkage rule used by
#'   the legacy cross-fitted inference routine.
#' @param mr_mashr_args,mr_ash_args Optional tuning lists forwarded to the
#'   `mr.ash` / `mr.mashr` backend when it is requested.
#'
#' @return A list containing replication-level outcomes, summarized coverage and
#'   false-discovery metrics, subspace recovery summaries, and the underlying
#'   fitted objects for each simulation.
#' @export
...
#' Write CSV and PDF reports for a simulation benchmark
#'
#' @param simulation_results Output from [run_rrr_uq_simulation()].
...
#'
#' @return A list of generated artifact paths.
#' @export
